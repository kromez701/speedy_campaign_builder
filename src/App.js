...
export const ProfileContext = createContext(); // Create a context for profile data
...
const App = () => {
  ...
  const [profilePic, setProfilePic] = useState('./assets/no-profile-picture-15257.svg'); // Manage profile pic
    ...
    const checkCurrentUser = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          setProfilePic(response.data.user.profile_picture || './assets/no-profile-picture-15257.svg'); // Set initial profile pic
        }
      } catch (error) {
  ...
  const handleAuthSuccess = async () => {
    try {
      ...
      if (response.status === 200) {
        setUser(response.data.user);
        setProfilePic(response.data.user.profile_picture || './assets/no-profile-picture-15257.svg'); // Set profile pic after login
        setAuthMode(null);
